# AD Attack Lab (Part 4) - Windows Forensic Investigation

Parts 1 through 3 were the attacker's story. This is the analyst's story.

Five alerts fired in Wazuh. An analyst just got assigned the ticket. No memory of what ran, no notes from the attack phase — just the logs, the tools, and the question: what actually happened here, and when?
...
The earliest cluster appears at **2026-03-30 21:32:45**. This is the first recorded hostile activity in the logs — the attacker's opening move.
...
One thing worth noting here: this event is dated the following day, distinct from the rest of the attack chain which ran on the evening of March 30. This is a real forensic observation — the AS-REP roast shows up as a separate, isolated event on a different day, which in a real investigation would prompt the question of whether the attacker returned, or whether this represents a separate tool run. In this lab context, it reflects the Phase 3 detection rule testing re-run rather than the original attack session.
...
The payload is unambiguous: `Target: CORP\Domain Admins`, `Member: CN=Ava Simmons, OU=Finance, DC=corp...` — asimmons promoted to Domain Admin.
...
The burst begins at 2026-03-30 23:54:14. The tight cluster of 4662 events visible in Timeline Explorer spans dozens of records at that timestamp — each one representing a portion of the directory replication pull from secretsdump.py.
...
No. All hostile activity was directed from 192.168.93.156 (Kali) straight to the DC. The W10 workstation logs show no indicators of compromise. The attacker had no need to pivot through the workstation — once valid credentials were obtained from the spray, the path to the DC was direct.
...
Four parts. One complete attack chain — from reconnaissance spray to full domain compromise — with detection rules that fire on the attacks and go quiet on normal traffic, and a forensic timeline built entirely from evidence.

The GitHub repo has everything: detection rules, the attack playbook, MITRE ATT&CK coverage, and this investigation writeup. All of it built by hand from raw logs — which is the point.
