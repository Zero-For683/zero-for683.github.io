...
This post covers Phase 3: building five custom Wazuh detection rules from scratch, one per attack technique, testing that each one fires correctly, and running benign activity to make sure they don't fire when they shouldn't. Getting that second part right is what separates a useful detection from one that cries wolf until someone disables it.
...
The rule solves this with a **frequency threshold**: it chains off Wazuh's built-in parent rule for NTLM failed logons, adds a filter for Logon Type 3 (network logon only, not interactive failures from the local machine), and only fires when 5 or more failures hit within a 60-second window.
...
- `if_matched_sid`: tells Wazuh to only evaluate the rule when a specific parent rule has already fired. Instead of parsing the raw log again, we're stacking on top of an existing match. It's more efficient and avoids duplicating logic.
...
The reason both rules exist is that a real adversary will often try multiple spray methods in sequence or in parallel. Covering only NTLM failures and missing the Kerberos path leaves a blind spot.
...
The AS-REP Roasting rule keys on Event ID 4768 (Kerberos authentication ticket request) with `preAuthType` set to `0`. Pre-auth type 0 means the DC responded without requiring the client to prove their password first. That's the misconfiguration that makes this attack possible.
...
Pass-the-Hash was the most interesting rule to tune. The detection approach keys on NTLM network logons where the target username resolves to `ANONYMOUS LOGON`. That pattern shows up during Pass-the-Hash attempts when the authentication handshake doesn't carry a real user identity in that field.
...
The 400-hit volume reflects the number of replication calls secretsdump.py makes when pulling the full domain hash database.

## False Positive Testing

Writing a rule that fires on an attack is only half the job. A rule that also fires on normal activity is worse than useless. It trains analysts to ignore it and eventually gets disabled. Each rule needs to be tested against the benign activity it might confuse for an attack.
...
Mapping a drive authenticates over Kerberos by default and generates network logon traffic. On paper, that could look like Kerberoasting or lateral movement. The rules should stay quiet.
...
With rules written, tested, and validated, Phase 4 is the forensic investigation. The task is working backwards from the alerts, treating them like a real ticket, and reconstructing what happened using only the logs. Eric Zimmerman's tools, Timeline Explorer, and six exported event log files are next.
